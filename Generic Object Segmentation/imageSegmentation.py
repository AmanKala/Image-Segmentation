...
from sklearn.metrics.pairwise import euclidean_distances      # Compute the distance matrix between each pair from a vector array X and Y.
...
for y in range(imageH):
    for x in range(imageW):
        image.putpixel((x, y), (
        int(dataVector[y * imageW + x][0]), int(dataVector[y * imageW + x][1]), int(dataVector[y * imageW + x][2])))
